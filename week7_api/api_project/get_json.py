...
def buildWebpage(who, oldCommentList, newCommentList):
	...
	relTag = soup.new_tag("link")
	relTag.attrs["rel"] = "stylesheet"
	relTag.attrs["href"] = "webpage.css"
	soup.head.append(relTag)

	mainDiv = soup.new_tag("div", id="oldComments")
	soup.body.append(mainDiv)
	head_tag = soup.new_tag("h1") # create a header tag
	head_tag.string = who + " old comments" # header tag will be person searched for, plus old or new comments
	mainDiv.append(head_tag) # put the header tag in our main div class
	...
	newDiv = soup.new_tag("div", id="newComments")
	soup.body.append(newDiv)

	head_tag = soup.new_tag("h1")
	head_tag.string = who + " new comments"
	newDiv.append(head_tag)
