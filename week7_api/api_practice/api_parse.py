...
def writeJsonFile(data):
	json.dump(data, open('wiki_parse_json.txt', 'w'), indent=4) #store our search in a file
...
def outPutWikiImg(data):
	...
	for title in sorted(wikiDict): 
		...
		for item in (wikiDict[title]['images']): # iterate through ID num dict by 'images'
